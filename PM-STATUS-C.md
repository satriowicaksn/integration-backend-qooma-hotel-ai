...
##### VERDICT FOUNDATION FIX Q-C-01 + Q-C-05 — RETRO-APPROVED by PM C (H23, 2026-07-08)

**Retro-posted after PR #25 merge** (commit `ff2d5e7` on main). Approving in retrospect for the audit trail; the fix was ratified by user directive at commit time + subsequent PR merge.

Independent verification (against current main state)
- `src/core/prisma/prisma-client.ts` on main: real `PrismaClient` singleton wired (no longer `{} as Record<string, unknown>` placeholder).
- `prisma/schema.prisma` on main: custom `output` line removed — resolves pnpm isolated-linker × custom-output interaction.
- CI Docker-build stage: transitioned RED → GREEN post-merge (verified via subsequent T17-followup PR #27 landing cleanly).
- All Prisma-touching slot-C followups (T17-fu merged, T20-fu + T23-fu + T19-fu APPROVED) now use real Prisma singleton via `db` import at `src/entrypoints/api-server.ts`.

Verdict
- **Scope**: precisely 2 files as declared (`prisma/schema.prisma` -1 line + `src/core/prisma/prisma-client.ts` unstub). Zero touches to `src/modules/**`, `src/plugins/**`, `package.json`, other slots.
- **Drift-scan clean** (verified retro against merged main state).
- **`make check` green** at merge time.
- **Cross-slot impact realized**: 8 T##-followup PRs unblocked (T17-fu was first to consume; T20-fu + T23-fu + T19-fu followed with `db` singleton import).

Housekeeping consequences
- Q-C-01 → **RESOLVED** in §3 (see updated row below).
- Q-C-05 → **RESOLVED** in §3.
- Q-C-02 (api.ts bootstrap) landed via PR #26 `41d25fb` → **RESOLVED** in §3.
- Q-C-03 (JWT plugin) landed via PR #26 `c0d0fb7` → **RESOLVED** in §3.
- Q-C-04 (tenant-id source) resolved by Q-C-03 outcome (JWT `payload.hotel_id` populates `req.hotelId` at `jwt-auth.plugin.ts:173`) → **RESOLVED** in §3.

Executor was authorized by user directive to ship the shared-infra fix; PM C validated at merge time via PR #25 CI-green + subsequent followup CI-green cascading. Approval is now formal in the audit trail.
...
| ------------- | -------- | -------------- | ------ | ---------- |
| Q-C-01 | **Prisma client singleton wiring — foundation gap; affects B+C.** `src/core/prisma/prisma-client.ts:29` still `export const db = {} as unknown as Record<string, unknown>` placeholder. Not delivered by any T01-T09 (PM A explicit precedent: primitives shipped, wiring deferred to "assembly"). Blocks any repository / integration test in T10 (B) + T17-T25 (C). **Ask Parent PM**: (a) route as slot-A foundation follow-up (F10?), or (b) authorize slot-B/C to ship (1-line uncomment; dep already declared). Preferred (a) for ownership clarity. | PM C (Satrio) H13 | src/core/prisma/prisma-client.ts:11-29; T17 PLAN GAP-#1 | **resolved** | PR #25 `ff2d5e7` (2026-07-07, user-directed shared-infra fix per FOUNDATION FIX §2 block) unstubbed `prisma-client.ts` to a real `PrismaClient` singleton with SIGTERM/SIGINT graceful shutdown. Consumed by T17-followup (PR #27 merged) + T20-fu + T23-fu + T19-fu wiring. PM C retro-VERDICT approved H23 2026-07-08 (see §2 block). |
| Q-C-02 | **`src/entrypoints/api.ts` bootstrap — foundation gap; affects all HTTP endpoints.** File still stub (line 38 `console.warn`). Fastify server + error-handler plugin (T08) + correlation-id + tenant-resolver (T05) + config load + graceful shutdown not wired. Blocks endpoint reachability for T10-T20 + T23. Q-A-05 (eslint async-hook `checksVoidReturn.properties: false`) recommended land **before** or bundled with this so all future async `preHandler`/hook code passes lint cleanly. **Ask Parent PM**: prioritize/assign api.ts bootstrap task (F11?). | PM C (Satrio) H13 | src/entrypoints/api.ts:11-45; T17 PLAN GAP-#2 | **resolved** | PR #26 `41d25fb` (2026-07-08) added `src/entrypoints/api-server.ts` factory + api.ts bootstrap. `buildServer()` composes error-handler + correlation-id + graceful shutdown; consumed by all followup route landings (T17-fu, T20-fu, T23-fu, T19-fu). |
| Q-C-03 | **Session/JWT auth plugin absent — cross-service contract Q; blocks all `gm_admin` CRUD (spec §2.1).** `src/plugins/` has hmac-validator + tenant-resolver + internal-rpc-auth + error-handler only. `env.ts:36-39` declares `JWT_ACCESS_SECRET`/`JWT_REFRESH_SECRET` but no plugin consumer. Auth service lives in separate repo (KICKOFF §1 L11); Integration verifies JWTs signed by Auth. Cross-service ratification needed: (a) verification method — JWKS URL fetch vs HS256 shared secret? (b) JWT payload shape — `{ sub, hotel_id, role, exp }`? (c) refresh-token flow — irrelevant to Integration (doesn't issue)? Preferred MVP: HS256 shared secret + `{ sub, hotel_id, role }` + verify-only plugin. Blocks T10 (B), T17 route (C), T18-T20, T23. **Ask Parent PM**: route to PO — cross-service contract. | PM C (Satrio) H13 | KICKOFF §1 L11; env.ts:36-39; src/plugins/*; T17 PLAN GAP-#3 | **resolved** | PR #26 `c0d0fb7` (2026-07-08) added `src/plugins/jwt-auth.plugin.ts` — HS256 verify-only (MVP choice) with `jwtAuthGuard({ secret })` + `requireRole(role)` composables. Payload shape `{ sub, hotel_id, role, exp, iat }` per JWT plugin implementation. Consumed by T17-fu (gm_admin route) + T23-fu (gm_admin overview route). Refresh flow not implemented (Auth svc handles issuance). |
| Q-C-04 | **Tenant identification for CRUD endpoints — cascading from Q-C-03.** Spec `/api/integrations/telegram` has no `:hotel_slug` in path (unlike webhook routes at `/webhooks/wa/:hotel_slug` spec §2.2). Alternatives: (a) JWT payload `hotel_id` (session-bound; preferred if Q-C-03 lands JWT); (b) header `X-Hotel-Id` (weak); (c) path rewrite `/api/hotels/:hotel_slug/...` (spec-drift). Locked to Q-C-03 outcome. | PM C (Satrio) H13 | spec §2.1 vs §2.2; T17 PLAN GAP-#4 | **resolved** | Option (a) shipped: `jwt-auth.plugin.ts:173` populates `req.hotelId = payload.hotel_id` at JWT verify. Route handlers extract via `requireHotelId(req.hotelId)` pattern. Consumed by T17-fu + T23-fu. |
| Q-C-05 | **Dockerfile × pnpm × Prisma custom-output — SHARED-INFRA BUG; main is currently RED.** Docker build fails on `pnpm build` (tsc -p tsconfig.build.json) with `TS2305: Module '@prisma/client' has no exported member 'TelegramConfig'` + `no exported member 'WaConfig'`. Locally + on CI unit/integration/lint/typecheck: green. Dockerfile stage 2 line 25 explicitly runs `pnpm prisma:generate`; schema.prisma:3 uses custom `output = "../node_modules/.prisma/client"`. Suspected pnpm strict-hoisting × prisma custom-output interaction (known upstream category). **T10 (PM B PR #10) merged RED with same failure at 2026-07-04T16:28:09Z**, meaning main has been red on Docker-build since T10 landed — this pre-dates T17. Precedent conflict with `PM-AGENT.md §4` "Merge tanpa lulus CI". PR #11 (T17) inherits + repeats same failure; not a T17 code defect. **Ask Parent PM**: (a) which fix candidate — remove custom output, add `.npmrc public-hoist-pattern`, or hoist step in Dockerfile? (b) route as slot-A shared-infra follow-up (F12?)? (c) merge-policy ratify for T10 + T17 pending fix (rollback T10 or waive Docker-build check batch-wide)? | PM C (Satrio) H14 (2026-07-05) | Dockerfile L22-32; prisma/schema.prisma:1-5; GH Actions run 28716832757 job Docker-build FAILURE | **resolved** | PR #25 `ff2d5e7` (2026-07-07) removed the custom `output` line from `prisma/schema.prisma`. Prisma now emits into default `node_modules/@prisma/client/.prisma/client/` — pnpm isolated linker resolves correctly. Docker-build stage transitioned RED → GREEN post-merge (verified via T17-followup PR #27 landing cleanly). PM C retro-VERDICT approved H23 2026-07-08. |
...
| Q-C-13 | **Telegram webhook secret storage — schema-level; blocks prod cutover once winston lands.** T19-followup MVP uses the decrypted `bot_token` as the value compared against `X-Telegram-Bot-Api-Secret-Token` (`TelegramWebhookSecretResolver`). Rationale: Telegram Bot API requires SOME secret at `setWebhook` time; the T17 primitive's `telegram_configs` schema has no dedicated `webhook_secret_enc` column (slot-B's WA HAS an analog `webhook_verify_token` per `prisma/schema.prisma:39` — grounds for parity). **Zero log-leak risk TODAY** because winston is a no-op stub (see Q-C-14). **Options**: (a) add `webhook_secret_enc String?` column to `telegram_configs` via a new Prisma migration (schema-shape change; slot-A/foundation domain); (b) `HMAC-SHA256(bot_token, hotel_id)` stateless derivation — no schema change; adds hash step at `setWebhook` operational tooling; (c) keep bot_token-as-secret + ensure winston redact list includes `x-telegram-bot-api-secret-token` (weakest — secret still traverses HTTP into our process). **Preferred**: (a) — parity with WA; explicit invariant. **Blocking**: prod cutover of T19-followup once winston gets wired (currently no-op) OR earlier if any log call captures the header. **NOT blocking**: T19-followup PR merge, primitives on main, integration testing (winston is no-op). Sibling to Q-C-11 (schema-level pending decision at spec/FE layer). | PM C (Satrio) H23 (2026-07-08) | spec §2.3; T19-followup PLAN GAP #2 + PM C ACK; slot-B `prisma/schema.prisma:39` precedent | open | — |
| Q-C-14 | **Winston logger is a NO-OP stub — foundation gap; blocks prod cutover.** `src/core/logger/logger.ts:32-38`: the `createLogger` factory returns an object where `debug/info/warn/error` are all `() => {}` (empty arrows). The 4 aspirational winston features are TODO comments only: JSON format, pretty print (dev), redact paths (`**/password`, `**/token`, `**/apiKey`, `**/wa_phone` masking, `**/email` masking), default meta (`service, version, env`). **Impact**: (1) production has ZERO logging today — no observability from application code. (2) Every `logger.*` call across the codebase is a compile-time contract that silently no-ops at runtime. (3) Test-level `logger.info.mock` assertions still work (mocks are injected per-test); production behavior differs. (4) When winston finally gets wired, the redact pattern list MUST include `x-telegram-bot-api-secret-token` header + `bot_token` field + `access_token_enc` shape (Q-C-13 dependency). **NOT blocking**: any followup approval TODAY. **Blocking**: prod cutover; SLA-affecting incident response; forensic audit. **Preferred fix**: wire winston per the TODO comments; add redact-list tests; document in README §2.3. Slot-A / shared-infra domain. Sibling to Q-A-08 (AppError codes drift). | PM C (Satrio) H23 (2026-07-08) | `src/core/logger/logger.ts:32-38` TODO block; T19-followup PM C VERDICT pre-approval findings | open | — |
| Q-C-15 | **`throw new Error(...)` at entrypoint boot/runtime-invariant guards — needs shared `BootstrapError extends AppError`.** Two occurrences accepted as tolerated deviations (per T20-followup + T19-followup VERDICTs): (i) T20-followup `api-server.ts:88` boot-guard for missing `INTERNAL_RPC_SECRET` when registering internal RPC routes; (ii) T19-followup `api-server.ts:125` runtime-invariant guard against Fastify preHandler-order regression. Both fall OUTSIDE the strict drift-scan scope (`src/modules/**/*.service.ts` + `src/core/`) so they don't formally violate the drift table, but CLAUDE.md §5 spirit prefers `AppError` subclasses everywhere. **Preferred fix**: introduce shared `BootstrapError extends AppError` (or `RuntimeInvariantError`) at `src/core/errors/app-errors.ts` — dedicated `code: 'BOOT_FAILURE'` / `code: 'INVARIANT_VIOLATED'`, `statusCode: 500`, structured message. Once class lands, replace the 2 known `throw new Error(...)` in entrypoints. Slot-A / shared-infra domain. **NOT blocking** — accepted tolerated deviations remain valid until the class arrives. | PM C (Satrio) H23 (2026-07-08) | `api-server.ts:88` (T20-followup) + `api-server.ts:125` (T19-followup) VERDICT tolerated-deviation blocks | open | — |
| Q-C-16 | **`INTERNAL_RPC_SECRET.optional()` in env schema — fixture-alignment sync note; long-term tighten to required.** T20-followup shipped `INTERNAL_RPC_SECRET: z.string().min(32).optional()` at `src/core/config/env.ts` as a workaround per T20-followup PLAN GAP #7 — required-schema breaks slot-B/A test fixtures which don't set the field (PM C cannot edit cross-slot fixtures under strict per-slot ownership). Wiring-time fail-fast at `api-server.ts:88` (`if (config.INTERNAL_RPC_SECRET === undefined) throw ...`) preserves prod safety. **Two-step path forward**: (a) slot A + slot B each add `INTERNAL_RPC_SECRET: 'x'.repeat(48)` (or similar) to their `beforeAll`/`resetConfigCache` setup in existing integration test files (whatsapp-*.integration.test.ts, etc.); (b) shared-infra chore tightens schema to `.min(32)` without `.optional()`. **NOT blocking** any followup approval. Slot-A + Slot-B coordination needed. | PM C (Satrio) H23 (2026-07-08) | T20-followup GAP #7 + PM C VERDICT process note PN #2 | open | — |
