...
| T21 | OTA email IMAP poller + parser pipeline + HC pending-visit RPC                   | wip (PLAN ACK'd) | PM C (H17, a1) | New module `src/modules/ota-mailbox/`. PLAN ACK'd H17: per-OTA parsers (Booking.com + Agoda) + dispatcher + Prisma-direct repo + poll orchestrator + 2 type-only ports (IMAP fetcher + HC pending-visit). Cron worker + adapters + integration = T21-followup. 13 binding conditions honored; Q-C-09 raised (HC pending-visit RPC contract). Q-C-05 Docker-red will apply. Coding proceeding |
