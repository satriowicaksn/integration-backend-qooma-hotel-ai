...
| T22 | QR generation + download (1024×1024 PNG, object storage)                         | wip (PLAN ACK'd) | PM C (H18, a1) | New module `src/modules/qr-provisioning/`. PLAN ACK'd H18: `wa.me` URL builder (module-private) + 2 type-only ports (QR renderer + object storage) + Prisma-direct repo + service orchestrator + zod schemas. Router+`pnpm add qrcode`+`pnpm add @aws-sdk/client-s3`+adapters+integration = T22-followup. 15 binding conditions; Q-C-10 raised (object storage contract). Q-C-05 Docker-red will apply. Coding proceeding |
