...
| T07 | Queue + scheduler infra (BullMQ-based, retry policy + DLQ)                       | A    | Nathan  | approved | PM A (H12)  | **Bull 4.x** (title "BullMQ" = misnomer), backoff [1s/5s/30s] attempts=3 configurable, DLQ `<mod>:dead` forwarder (exhaustion-gated), Redis-injected, logic 100% cov / no open handles. Awaiting PO merge. Consumed by T14+T19+T21+T24. Q-A-07 (retry count) |
